...
const AUTHORIZED_ADMIN_EMAIL = "hapa1929@gmail.com";
...
// ----------------------------------------------------
// 8. GOOGLE AUTHENTICATION & ACCESS CONTROL
// ----------------------------------------------------
let hasInitializedStreams = false;
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    ...
    if (userEmail === AUTHORIZED_ADMIN_EMAIL.toLowerCase()) {
      // 1. Authorized Admin (hapa1929@gmail.com): Grant Full Access
      if (loginPortal) loginPortal.style.display = "none";
      if (mainDashboard) mainDashboard.style.display = "block";
      if (adminUserEmail) adminUserEmail.textContent = user.email;
      if (loginErrorMsg) loginErrorMsg.style.display = "none";

      showToast(`Welcome Administrator (${user.email})!`);
      recordLog("Auth Security", user.email, "/auth/login/success", 200, "Admin Verified");

      // Initialize streams only once per session
      ...
      // 2. Unauthorized Account: Block and sign out immediately
      await signOut(auth);
      ...
      if (loginErrorMsg) {
        ...
        loginErrorMsg.innerHTML = `
          <strong>🚫 ACCESS DENIED & REMOVED</strong><br>
          Account <code>${userEmail}</code> is NOT authorized to access this platform.<br>
          Only <code>${AUTHORIZED_ADMIN_EMAIL}</code> can access this system. You have been disconnected immediately.
        `;
      }
      showToast(`Unauthorized login blocked: ${userEmail}`, true);
    }
  } else {
    // 3. No User Logged In: Only show login portal
    if (mainDashboard) mainDashboard.style.display = "none";
    if (loginPortal) loginPortal.style.display = "flex";
  }
});
...
document.getElementById("googleLoginBtn")?.addEventListener("click", async () => {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = "⏳ Authenticating with Google...";
  }
  ...
  try {
    ...
    if (errBox) {
      ...
      errBox.innerHTML = `⚠️ <strong>Authentication Error:</strong> ${err.message}`;
    }
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `
...
document.getElementById("signOutBtn")?.addEventListener("click", async () => {
  if (!confirm("Are you sure you want to sign out of CyberAuth Authority?")) return;
  hasInitializedStreams = false;
  await signOut(auth);
